...
driverUrl = driverConfig["driverNiblUrl"]
    ...
    for row in downloadList:
        ...
        if not done:
            ...
            while True:
                ...
                botPackList.append([botName, xdccPack, dir_name, episode, current_season, bytes(image)])
...
def downloadAnimeFromList(animeListToDownload, printOutput=False):
    conn, cursor = getDbConnAndCursor(sqlServerName, database)
    if conn == 2:
        sys.exit(cursor)
    downloadCounter = 0
...
if __name__ == "__main__":
    ...
    getListToDownload = getAnimeListFromDb(printOutput=True, downloadCurrentDay=False)
    # print(getListToDownload) # [['Ginpachi-Sensei', '5288', 'Shinka no Mi Shiranai Uchi ni Kachigumi Jinsei', 5, 2], ['Ginpachi-Sensei', '5436', 'Tomo-chan wa Onnanoko!', 7, 1]]
